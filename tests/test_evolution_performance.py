...
def iterative_backtester(df: pd.DataFrame, genome: StrategyGenome) -> float:
    cash = 10000.0
    position = 0.0
    for i in range(1, len(df)):
        buy = (genome.dna['feature_rsi_enabled'] and df['rsi'].iloc[i-1] < genome.dna['rsi_threshold_buy'])
        sell = (genome.dna['feature_rsi_enabled'] and df['rsi'].iloc[i-1] > genome.dna['rsi_threshold_sell'])
...
class TestEvolutionPerformance(unittest.TestCase):
    ...
    def test_vectorized_backtester_correctness(self):
        ...
        # Simplify the genome for a predictable test
        genome.dna['feature_rsi_enabled'] = True
        genome.dna['rsi_threshold_buy'] = 30
        genome.dna['rsi_threshold_sell'] = 70
        genome.dna['feature_macd_enabled'] = False # Disable other features for this test

        # Calculate fitness using both methods
        vectorized_fitness = self.evolution._evaluate_fitness_vectorized(genome)
        iterative_fitness = iterative_backtester(self.historical_data, genome)
